package de.thegerman.simplesafe.views
...
class CirclePageIndicator @JvmOverloads constructor(
    ...
    private val inactivePaint = Paint(activePaint).apply {
        color = context.getColorCompat(R.color.pressedShade)
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val activeColor = activePaint.color
        activePaint.color = ColorUtils.blendARGB(activeColor, inactivePaint.color, currentPositionOffset)
        canvas.drawCircle(xOffset + cx, yOffset, radius, activePaint)

        if (currentPositionOffset > 0) {
            activePaint.color = ColorUtils.blendARGB(inactivePaint.color, activeColor, currentPositionOffset)
            canvas.drawCircle(xOffset + cx + itemSpace, yOffset, radius, activePaint)
        }
